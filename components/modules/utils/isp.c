...
void yuv422p_to_gray(uint8_t *pdst, const uint8_t *psrc, int h, int w, int th, int tw, uint8_t rotation)
{
    int32_t y;
    int32_t cr;
    int32_t cb;
    ...
    for (int i = 0; i < th; i++)
    {
        for (int j = 0; j < tw; j++)
        {
            ...
            pdst[index] = ((uint8_t)CLIP(r) * 299 + (uint8_t)CLIP(g) * 587 + (uint8_t)CLIP(b) * 114) / 1000;
        }
    }
}
