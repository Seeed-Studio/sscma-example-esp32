...
using registered_sensor_t = std::forward_list<el_sensor_t>;
...
void at_set_model(uint8_t model_id, uint8_t& current_model_id, InferenceEngine* engine) {
    auto*           serial        = Device::get_device()->get_serial();
...
void at_get_available_sensors(const registered_sensor_t& registered_sensors) {
    auto*  serial = Device::get_device()->get_serial();
    auto   os     = std::ostringstream(std::ios_base::ate);
...
void at_set_sensor(uint8_t              sensor_id,
                   bool                 enable,
                   uint8_t&             current_sensor_id,
                   registered_sensor_t& registered_sensors) {
    auto* device  = Device::get_device();
    auto* serial  = device->get_serial();
    auto* storage = DataDelegate::get_delegate()->get_storage_handler();
    auto  os      = std::ostringstream(std::ios_base::ate);
    auto  it      = std::find_if(registered_sensors.begin(), registered_sensors.end(), [&](const el_sensor_t& sensor) {
...
void at_run_sample(uint8_t sensor_id, const registered_sensor_t& registered_sensors) {
    auto* device = Device::get_device();
    auto* serial = device->get_serial();
    auto  it     = std::find_if(registered_sensors.begin(), registered_sensors.end(), [&](const el_sensor_t& sensor) {
        return sensor.id == sensor_id;
...
template <typename T>
void run_invoke_algorithm(InferenceEngine*   engine,
                          int                n_times,
...
void at_run_invoke(InferenceEngine*           engine,
                   int                        n_times,
                   std::atomic<bool>&         stop_token,
                   uint8_t                    current_algorithm_id,
                   uint8_t                    current_model_id,
                   uint8_t                    current_sensor_id,
                   const registered_sensor_t& registered_sensors) {
    auto*               serial             = Device::get_device()->get_serial();
    auto*               algorithm_delegate = AlgorithmDelegate::get_delegate();
    auto*               models             = DataDelegate::get_delegate()->get_models_handler();
    auto                os                 = std::ostringstream(std::ios_base::ate);
    ...
    auto                sensor_config_it =
      std::find_if(registered_sensors.begin(), registered_sensors.end(), [&](const el_sensor_t& sensor) {
