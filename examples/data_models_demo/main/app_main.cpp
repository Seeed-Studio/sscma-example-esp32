...
extern "C" void app_main() {
    ...
    auto* models = edgelab::Models::get_ptr();
    ...
    for (const auto& i : all_model_info)
        printf("\tmodel: {id: %d, type: %d, flash_address: 0x%.8lX, size: %ld, memory_address: %p}\n",
               i.id,
               i.type,
               i.addr_flash,
               i.size,
    ...
    printf("Seek all plain TFLite model info again from flash and print (default is packed TFLite "
           "mode) ->\n");
    ...
    printf("Seek all packed TFLite model info again from flash and print (faster than plain TFLite "
           "mode) ->\n");
