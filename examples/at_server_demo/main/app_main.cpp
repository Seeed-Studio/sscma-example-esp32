    ...
    *storage << el_make_storage_kv("boot_count", ++boot_count);
    ...
    instance->register_cmd("ALGO",
                           ...
                           el_repl_cmd_write_cb_t([&](int argc, char** argv) -> el_err_code_t {
                               ...
                               if (algorithm_delegate->has_algorithm(algorithm_id))
    ...
    instance->register_cmd(
      ...
      el_repl_cmd_exec_cb_t([&]() -> el_err_code_t {
          ...
          if (it->type == 1u && camera && static_cast<bool>(*camera)) {
              ...
              ret = EL_EINVAL;
      SampleReplyError:
          os << "{\"sensor_id\": " << unsigned(current_sensor_id) << ", \"type\": " << unsigned(found ? it->type : 0u)
      ...
      el_repl_cmd_write_cb_t([&](int argc, char** argv) -> el_err_code_t {
          ...
          static auto thr = std::thread([&]() {
              ...
              case el_algorithm_type_t::ALGORITHM_YOLO:
                  if (!algorithm) algorithm = new AlgorithmYOLO(engine);

                  while (n_times < 0 || --n_times > 0) {
                      ret = camera->start_stream();
                      if (ret != EL_OK) [[unlikely]]
                          break;
                      ret = camera->get_frame(img);
                      ...
                      if (ret != EL_OK) [[unlikely]]
                          break;
                      preprocess_time  = algorithm->get_preprocess_time();
                      run_time         = algorithm->get_run_time();
                      postprocess_time = algorithm->get_postprocess_time();
                      uint8_t i        = 0u;
                      ...
                      os << "{\"algorithm_id\": " << unsigned(current_algorithm_id)
                         ...
                         << el_results_2_json(static_cast<AlgorithmYOLO*>(algorithm)->get_results()) << ", \"data\": \""
                         << el_img_2_base64_c_str(img) << "\"}\n";
                      auto str = os.str();
                      serial->write_bytes(str.c_str(), str.size());
                  }
    ...
    // // setup components
    instance->loop("\n", 2);
    instance->loop("AT+ALGO=3\n", 11);
    instance->loop("AT+MODEL=1\n", 12);
    instance->loop("AT+SENSOR=1\n", 13);
