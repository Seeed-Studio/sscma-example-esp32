...
extern "C" void app_main(void) {
    ...
    *storage << el_make_storage_kv("boot_count", ++boot_count);
    ...
    instance->register_cmd("ID", "Get device ID", "", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
                               executor->add_task([&](std::atomic<bool>& stop_token) { at_get_device_id(); });
                               return EL_OK;
                           }));

    instance->register_cmd("NAME", "Get device name", "", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
                               executor->add_task([&](std::atomic<bool>& stop_token) { at_get_device_name(); });
                               return EL_OK;
                           }));
    ...
    instance->register_cmd(
      "VERSION", "Get version details", "", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          executor->add_task([&](std::atomic<bool>& stop_token) { at_get_version(); });
          return EL_OK;
      }));
    ...
    instance->register_cmd(
      "ALGO?", "Get available algorithms", "", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          executor->add_task([&](std::atomic<bool>& stop_token) { at_get_available_algorithms(); });
          return EL_OK;
      }));
    ...
    instance->register_cmd(
      ...
      el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          ...
          executor->add_task([&, algorithm_id = std::move(algorithm_id)](std::atomic<bool>& stop_token) {
    ...
    instance->register_cmd(
      "MODEL?", "Get available models", "", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          executor->add_task([&](std::atomic<bool>& stop_token) { at_get_available_models(); });
          return EL_OK;
      }));
    ...
    instance->register_cmd(
      "MODEL", "Load a model by model ID", "MODEL_ID", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          ...
          executor->add_task([&, model_id = std::move(model_id)](std::atomic<bool>& stop_token) {
    ...
    instance->register_cmd(
      "SENSOR?", "Get available sensors", "", el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          executor->add_task([&](std::atomic<bool>& stop_token) { at_get_available_sensors(registered_sensors); });
          return EL_OK;
      }));
    ...
    instance->register_cmd("SENSOR",
                           ...
                           el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
                               ...
                               executor->add_task([&, sensor_id = std::move(sensor_id)](std::atomic<bool>& stop_token) {
    ...
    instance->register_cmd(
      ...
      el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          uint8_t sensor_id = std::atoi(argv[1]);
          executor->add_task([&](std::atomic<bool>& stop_token) { at_run_sample(sensor_id, registered_sensors); });
          return EL_OK;
      }));
    ...
    instance->register_cmd(
      ...
      el_repl_cmd_cb_t([&](int argc, char** argv) -> el_err_code_t {
          ...
          executor->add_task([&, n_times = std::move(n_times)](std::atomic<bool>& stop_token) mutable {
    ...
    // setup components
    {
        ...
        os << "AT+ALGO=" << std::to_string(current_algorithm_id) << "\n";
